...
(function($) {
	$.fn.smart3d = function(settings) {
		...
		if (settings['horizontal']) 
			lis.css('left', (width - settings['frameWidth']) / 30);
		else lis.css('left', '0');
			
		if (settings['vertical']) 
			lis.css('top', (height - settings['frameHeight']) / 30);
		else lis.css('top', '0');
		...
		thisObj.mousemove(function(e){
			if (settings['horizontal']) {
				...
				if (settings['invertHorizontal'])
					thisObj.pos_x = offset_x / 30 - (x / width * offset_x);
				else
					thisObj.pos_x = (x / width * offset_x) - offset_x / 30;
			}
			if (settings['vertical']) {
				...
				if (settings['invertVertical']) 
					thisObj.pos_y = offset_y / 30 - (y / height * offset_y);
				else
					thisObj.pos_y = (y / height * offset_y) - offset_y / 30;
			}
		});
